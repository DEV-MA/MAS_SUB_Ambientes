...
init -990 python:
    store.mas_submod_utils.Submod(
        ...
        name="Habitación Personalizada Amueblado Espacio V2",
        description="Este submod añade un juego completo de muebles Spaceroom para ti y Monika.",
        version="1.0.5.1"
    )
...
init -989 python:
    if store.mas_submod_utils.isSubmodInstalled("Submod Updater Plugin"):
        store.sup_utils.SubmodUpdater(
            submod="Habitación Personalizada Amueblado Espacio V2",
            user_name="DEV-MA",
            repository_name="Ambiente_Sala-de-estar2",
            update_dir="",
            attachment_id=None
        )
...
label Furnished_spaceroom2_switch_dlg:
    python:
        switch_quip = renpy.substitute(renpy.random.choice([
            "Las plantas colgantes son encantadoras~",
            "¡Así que tantos libros!",
            "Me encantaría abrazarte en el sofá...",
        ]))
...
label bg_room_installed_low_affection:
    ...
    if spacerooms_installed:
        m 1wud "H-huh? {w=0.5} [player], {w=0.2} ¿Agregaste nuevos archivos al juego?"
        m 1wua "Veamos que es... {w=0.5} {nw}"
